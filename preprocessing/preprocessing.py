...
#  Funckja pomocnicza wyciągają nazwy kolumn po użyciu ColumnTransformera,
#  znaleziona na StackOverFlow.
...
def transform(text):
    """
    Funkcja przekształcająca dany rekord z kolumny amenities do listy zawierającej
    wszystkie możliwości z pierwotnej postaci.
    """
    lex = text.split(",")
    out = []
    for item in lex:
...
def extract_amenities(data, test_data=False):
    """
    Funkcja przekształcająca kolumnę amenities.
    """
    data_copy = data.copy()
...
def preprocess_data(data, test_data=False):
    """
    Funkcja wykonywująca preprocessing danych.

    Parameter:
        - test_data [boolean] -> służy do określania czy chcemy wykonać preprocessing dla zbioru
                                 treningowegy czy testowego
    """
    ...
    data["host_response_rate"] = (
        data["host_response_rate"].str.rstrip("%").astype("float")
    )

    data.host_has_profile_pic = np.where(
        data.host_has_profile_pic.isnull(), "f", data.host_has_profile_pic
    )
    data.host_identity_verified = np.where(
        data.host_identity_verified.isnull(), "f", data.host_identity_verified
    )
